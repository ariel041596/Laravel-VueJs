            ...
            <div class="col-sm-12">
              ...
              >
                ...
                <tbody>
                  ...
                  <!-- <template v-else> -->
                  <tr v-for="officer in property_officers.data" :key="officer.id">
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5
              ...
              class="modal-title"
              id="addNewModalLabel"
            >Add New Property Officer</h5>
            <h5 v-show="editmode" class="modal-title" id="addNewModalLabel">Update Property Officer</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span class="modal-close-button" aria-hidden="true">&times;</span>
            </button>
          </div>
          <form @submit.prevent="editmode ? updatePropertyOfficer() : createPropertyOfficer()">
...
export default {
  ...
  methods: {
    ...
    getResults(page = 1) {
      axios.get("api/property_officer?page=" + page).then(response => {
        this.property_officers = response.data;
      });
    },
    updatePropertyOfficer() {
      this.$Progress.start();
      this.form
        .put("api/property_officer/" + this.form.id)
        .then(() => {
          $("#addNew").modal("hide");
          toast.fire({
            type: "success",
    ...
    deletePropertyOfficer(id) {
      swal
        ...
        .then(result => {
          ...
          if (result.value) {
            this.form
              .delete("api/property_officer/" + id)
              .then(() => {
                this.$Progress.start();
    ...
    createPropertyOfficer() {
      // Progressbar before create user
      this.$Progress.start();
      this.form
        .post("api/property_officer")
        .then(() => {
