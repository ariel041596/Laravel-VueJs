              ...
              >
                ...
                <tbody>
                  <template v-if="assets.data<=0">
                    <tr>
                      <td colspan="15" class="text-center">No Items for Disposal Available</td>
                    </tr>
                  </template>
                  <template v-else>
...
export default {
  ...
  methods: {
    ...
    loadAssets() {
      if (this.$gate.isAdminOrUserOrAuthor()) {
        axios.get("api/disposal").then(({ data }) => (this.assets = data)); //Remove the previous (this.users =data.data) into data only
      }
    },
